...
check_token() {
  ...
  if [ "$USER_TOKEN" = "kyzosan" ]; then
    echo -e "${GREEN}AKSES BERHASIL${NC}}"
  else
    echo -e "${GREEN}Token Salah! Beli Kode Token Di Kyzosan${NC}"
    echo -e "${YELLOW}TELEGRAM : @kyzosan${NC}"
    echo -e "${YELLOW}YT : @rvmzngaceng${NC}"
    echo -e "${YELLOW}HARGA TOKEN : 25K FREE UPDATE JIKA ADA TOKEN BARU${NC}"
    echo -e "${YELLOW}© Kyzosan${NC}"
    exit 1
  fi
  clear
}
...
while true; do
  ...
  echo -e "${RED}'ggg      ,gg'''  .    ggg       Auto Installer Kyzosan   ${NC}"
  echo -e "${RED}gggg      gg     ,     ggg      ------------------------  ${NC}"
  echo -e "${RED}ggg:     gg.     -   ,ggg       • Telegram : Kyzosan      ${NC}"
  echo -e "${RED} ggg:     ggg._    _,ggg        • Creadit  : Kyzosan  ${NC}"
  echo -e "${RED} ggg.    '.'''ggggggp           • Support by Kyzosan  ${NC}"
